pub struct WeekTemperatures {
    // An array of 7 slots. 
    // Option<i32> allows us to store 'None' (no temp yet) or 'Some(val)'.
    temps: [Option<i32>; 7],
}
...
impl WeekTemperatures {
    pub fn new() -> Self {
        WeekTemperatures {
            // Initialize all 7 slots to None
            temps: [None; 7],
        }
    }

    pub fn get_temperature(&self, day: Weekday) -> Option<i32> {
        let index = self.day_to_index(day);
    ...
    pub fn set_temperature(&mut self, day: Weekday, temperature: i32) {
        let index = self.day_to_index(day);
        // Store the temperature at that specific index
        self.temps[index] = Some(temperature);
    }

    // Helper method to convert the Enum into a usable array index (0-6)
    fn day_to_index(&self, day: Weekday) -> usize {
        match day {
            Weekday::Monday => 0,
            Weekday::Tuesday => 1,
            Weekday::Wednesday => 2,
